...
<div class="p-2 grid grid-cols-1 md:grid-cols-3 gap-4 border rounded-lg bg-white rx-keep">
  ...
  {{-- MIDDLE: Doctor + Visit info (only if available) --}}
  <div class="text-sm space-y-1">
    @php
      $doctorName  = optional($prescription->doctor)->name;
      $doctorSpec  = optional($prescription->doctor)->specialization;
    ...
    <div class="flex flex-wrap gap-x-6 gap-y-1 text-gray-700">
          ...
          {{-- <span>Gurdian: <span class="font-medium">{{ $phoneText }}</span></span> --}}
          <span>Gurdian: <span class="font-medium">Here Show Gurdian Name </span></span>
        
        
          {{-- <span>Address: <span class="font-medium">{{ $emailText }}</span></span> --}}
          <span>Address: <span class="font-medium">Here Show Address</span></span>
        
      </div>

    {{-- @if($retText)
      <div>Return Date: <span class="font-medium">{{ $retText }}</span></div>
    @endif --}}
    ...
    {{-- You can add more identifiers here if you like --}}
    {{-- @if($pat?->blood_group) <div>Blood Group: <span class="font-medium">{{ $pat->blood_group }}</span></div> @endif --}}
  </div>

  {{-- RIGHT: Rx ID + Date (+ optional phone again) --}}
  <div class="text-sm text-left md:min-w-[200px] space-y-1">
    {{-- @if($showRxId)
      <div class="text-gray-500">Prescription #
        <span class="font-medium text-black">{{ $prescription->id }}</span>
      </div>
    @endif --}}
...
<div class="leading-none">{!! $barcodeSvg !!}</div>
        ...
        <div class="mt-6 grid grid-cols-12 gap-6">
            ...
            <div class="col-span-12 md:col-span-8">
                ...
                {{-- BARCODE + Signature: show only if we have an ID --}}
...
@media print {
  ...
  .rx-keep, .text-right, .border, .rounded-lg, .p-4, .p-4, .p- { page-break-inside:avoid; }
